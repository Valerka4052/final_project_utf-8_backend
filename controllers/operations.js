...
const searchRecipes = async (req, res) => {
  ...
  const { _id } = req.user;
  const skip = page - 1;
  const recipesLength = await Recipe.find({
...
const getRecipesByIngredient = async (req, res) => {
  ...
  const recipesLength = await Recipe.find({
    ingredients: { $elemMatch: { id: { $in: ingrIds } } },
  });
  const totalPages = Math.ceil(recipesLength.length / limit);
  const recipes = await Recipe.find({
    ingredients: { $elemMatch: { id: { $in: ingrIds } } },
...
const getFavoriteRecipeByUser = async (req, res) => {
  ...
  const { id } = req.user;
  const skip = page - 1;
  const recipes = (await Recipe.find({ favorite: { $elemMatch: { id } } }))
    .length;
  const totalPages = Math.ceil(recipes / limit);
  const userRecipes = await Recipe.find({ favorite: { $elemMatch: { id } } })
    .skip(skip)
    .limit(limit);
