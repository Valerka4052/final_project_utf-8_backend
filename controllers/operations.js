...
const addRecipeToFavorite = async (req, res) => {
    ...
    const updatedReciepe = await Recipe.findByIdAndUpdate(resId, { $push: { favorite: { _id } } }, { new: true });
...
const deleteRecipeFromFavorite = async (req, res) => {
    const { _id } = req.user;
    const { id: resId } = req.body;
    const deletedFromFavorite = await Recipe.findByIdAndUpdate(resId, { $pull: { favorite: { _id: _id } } }, { new: true });
...
const removeProductFromSoppingList = async (req, res) => {
    const { _id: userId } = req.user;
    const { id } = req.body;
    const updatedUser = await User.findByIdAndUpdate(userId, { $pull: { shoppingList:  id } }, { new: true });
    if (!updatedUser) throw HttpError(404, 'not found');
    return res.status(200).json(updatedUser.shoppingList);
};
...
const getUserInfo = async (req, res) => {
    ...
    const userRecipes = await Recipe.find({ favorite: { $elemMatch: { _id } } }).populate('favorite');
